...
% Button, FFT
hButtonFFT = uicontrol('Parent',hPanelOptionalPlots,...
    'Style','pushbutton',...
    'Units','normalized',...
    'Position',[0.05,0.76,0.9,0.08],...
    'String','FFT ECG',...
    'FontSize',10,...
    'Enable','on',...
    'Callback',@fftFunc);

% Text label, for Optional Plots panel
uicontrol('Parent',hPanelOptionalPlots,...
    'Style','text',...
    'Units','normalized',...
    'position',[0.05,0.66,0.9,0.05],...
    'HorizontalAlignment','left',...
    'String','Patience my young padawan...',...
    'FontWeight','normal',...
    'ForegroundColor',[0 0 0],...
    'FontSize',8,...
    'BackgroundColor',panelColor);
...
if strcmp(timeBase,'World')
    ...
    rangeStr = sprintf('%04d/%02d/%02d, %02d:%02d:%02d - %04d/%02d/%02d, %02d:%02d:%02d',Y(1),M(1),D(1),h(1),m(1),round(s(1)),Y(2),M(2),D(2),h(2),m(2),round(s(2)));
else
    if strcmp(signalName,'ECG')
        [h,m,s] = hms([xAxisTimeStamps.duration.ECG(rangeStartIndex) xAxisTimeStamps.duration.ECG(rangeEndIndex)]);
    elseif strcmp(signalName,'Resp')
