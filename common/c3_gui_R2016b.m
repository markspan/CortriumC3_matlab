...
%             set(hAxECGbrowser(ia),'Yticklabel',[]);
    ...
    function ecgWinFunc(varargin)
        ...
        hAxesECGWindow.XLim = [xAxisTimeStamp(rangeStartIndex.ECG) xAxisTimeStamp(rangeEndIndex.ECG)];
    end
    ...
    function respWinFunc(varargin)
        ...
        hAxesRespWindow.XLim = [xAxisTimeStamp(rangeStartIndex.Resp) xAxisTimeStamp(rangeEndIndex.Resp)];
    ...
    function accelWinFunc(varargin)
        ...
        hAxesAccelWindow.XLim = [xAxisTimeStamp(rangeStartIndex.Accel) xAxisTimeStamp(rangeEndIndex.Accel)];
        xlabel(['time (' timeBase ')'],'FontSize',10);
    ...
    function tempWinFunc(varargin)
        ...
        hAxesTempWindow.XLim = [xAxisTimeStamp(rangeStartIndex.Temp) xAxisTimeStamp(rangeEndIndex.Temp)];
...
%         hAxesECG.XLim = [datenum(xAxisTimeStamp(startIdx)) datenum(xAxisTimeStamp(endIdx))];
        hAxesECG.XLim = [xAxisTimeStamp(startIdx) xAxisTimeStamp(endIdx)];
    end
end
end
...
if get(hRespCheckbox, 'Value') == 1 % if checkbox for Resp is ON
    ...
    hAxesResp.XLim = [xAxisTimeStamp(startIdx) xAxisTimeStamp(endIdx)];
else
    cla(hAxesResp);
end
end
...
if sum(size(C3.accel)) ~= 0
    ...
    if (hAccelXCheckbox.Value == 1) || (hAccelYCheckbox.Value == 1) || (hAccelZCheckbox.Value == 1) || (hAccelMagnitudeCheckbox.Value == 1)
        ...
        hAxesAccel.XLim = [xAxisTimeStamp(startIdx) xAxisTimeStamp(endIdx)];
    end
end
end
...
if sum(size(C3.temp)) ~= 0
    ...
    if (hTemp1Checkbox.Value == 1) || (hTemp2Checkbox.Value == 1)
        ...
        hAxesTemp.XLim = [xAxisTimeStamp(startIdx) xAxisTimeStamp(endIdx)];
    end
...
%function called when loading new sensor data. Fills C3 button press events into eventMarkers.
...
function fftEcgNewWindow(C3,startIndex,endIndex,xAxisTimeStamps,timeBase,hSaveImagesCheckbox,hImageRes,hOptPlotRangeButtonGroup,gS,full_path)
    if gS.dataLoaded
        ...
        for ii=1:size(C3.ecg.data,2)
            ...
            xticks(hAx,linspace(min(xticks(hAx)), max(xticks(hAx)), length(xticks(hAx))*2-1));
            hold(hAx,'on');
