...
% Text-edit for Min XLim of Accel mag histogram
hAccMagHistXLimMin = uicontrol('Parent',hPanelAccMag,...
    'Style','edit',...
    'Enable','on',...
    'Units','normalized',...
    'Position',[0.66,0.6,0.12,0.28],...
    'String','0.8',...
    'HorizontalAlignment','center',...
    'FontSize',8,...
    'BackgroundColor',editColor);
...
% Text-edit for Max XLim of Accel mag histogram
hAccMagHistXLimMax = uicontrol('Parent',hPanelAccMag,...
    'Style','edit',...
    'Enable','on',...
    'Units','normalized',...
    'Position',[0.84,0.6,0.12,0.28],...
    'String','1.2',...
    'HorizontalAlignment','center',...
    'FontSize',8,...
    'BackgroundColor',editColor);
...
function accMagNewWindow(C3,startIndex,endIndex,xAxisTimeStamps,timeBase,hSaveImagesCheckbox,hOptPlotRangeButtonGroup,gS,hAccMagHistBinCount,hAccMagHistXLimMin,hAccMagHistXLimMax,full_path,jsondata)
    if gS.dataLoaded
        ...
        if xLimMin < 0 || xLimMax < 0
            if xLimMin < 0
                warndlg('XLim min must be >= 0');
                set(hAccMagHistXLimMin,'String','0');
                xLimMin = 0;
                return;
            end
            if xLimMax <= 0
                warndlg('XLim max must be > 0');
                set(hAccMagHistXLimMin,'String',num2str(xLimMin+2.0));
                xLimMin = xLimMin+2.0;
                return;
            end
        end
        if xLimMin >= xLimMax
            warndlg('XLim min must be < max');
            return;
        end
        if binCount < 1
        ...
        if hSaveImagesCheckbox.Value == 1
            saveImages = true;
            [source_path,~,~] = fileparts(full_path);
            imgFiles_path = [source_path filesep 'Images - Accel magnitude hist'];
            if ~exist(imgFiles_path,'dir')
                mkdir(imgFiles_path);
            end
        else
        ...
        winXpos = 100; winYpos = 100; winWidth = 1280; winHeight = 720;        
        paperSize = [(winWidth/72)*2.54 (winHeight/72)*2.54];
        paperPosition = [0 0 (winWidth/72)*2.54 (winHeight/72)*2.54];
        ...
        hAx.FontSize = 12;
        xlabel(hAx,'Acceleration magnitude (g-force)','FontSize',12);
        ylabel(hAx,'Normalized sample count (sum = 1)','FontSize',12);
        nameStr = '';
        if ~isempty(jsondata)
        ...
        title(hAx,sprintf('Acceleration magnitude\n%s   %s',nameStr, rangeStr),'FontSize',14,'FontWeight','bold');
        if saveImages
