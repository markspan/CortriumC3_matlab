...
%         linkaxes([hAxesECG hAxesResp hAxesAccel hAxesTemp], 'off');
        plotECG(C3,rangeStartIndex.ECG,rangeEndIndex.ECG,xAxisTimeStamps,timeBase,hAxesECG,hECG1Checkbox,hECG2Checkbox,hECG3Checkbox)
        if ~isempty(C3.resp)
...
function plotECG(C3,startIdx,endIdx,xAxisTimeStamps,timeBase,hAxesECG,hECG1Checkbox,hECG2Checkbox,hECG3Checkbox)
    xAxisTimeStamp = getTimeStamps(xAxisTimeStamps,timeBase,'ECG');
    plotOptions = getPlotOptions(timeBase);
...
function [eventECG_rowIdx, eventECG_colIdx] = getECGeventIndex(C3,rangeStartIdx,rangeEndIdx,hPopupEvent,searchDir)
    ...
    if strcmp(searchDir,'right')
        ...
        if hPopupEvent.Value == 2
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:)) >= 32765, 1);
        % if search for 'ECG Sample max value (ecg = 32767)'
        elseif hPopupEvent.Value == 3
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:) == 32767, 1);
        % if search for 'ECG Sample min value (ecg = -32765)'
        elseif hPopupEvent.Value == 4
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:) == -32765, 1);
        % if search for 'ECG Filter error (ecg = -32766)'
        elseif hPopupEvent.Value == 5
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:) == -32766, 1);
        ...
        elseif hPopupEvent.Value == 7
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:) == -32768, 1);
        % if search for 'ECG abs(ecg) >= 5000 & < 10000'
        elseif hPopupEvent.Value == 8
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:)) >= 5000 & abs(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:)) < 10000, 1);
        % if search for 'ECG abs(ecg) >= 10000 & < 20000'
        elseif hPopupEvent.Value == 9
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:)) >= 10000 & abs(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:)) < 20000, 1);
        % if search for 'ECG abs(ecg) >= 20000'
        elseif hPopupEvent.Value == 10
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(rangeEndIdx:C3.ecg.samplenum,:)) >= 20000, 1);
        end
        if ~isempty(eventECG_rowIdx)
    ...
    % else, search to the left side of the currently displayed data
        ...
        % if search for 'ECG Error Code (any error)'
        if hPopupEvent.Value == 2
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(1:rangeStartIdx,:)) >= 32765, 1, 'last');
        % if search for 'ECG Sample max value (ecg = 32767)'
        elseif hPopupEvent.Value == 3
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(1:rangeStartIdx,:) == 32767, 1, 'last');
        % if search for 'ECG Sample min value (ecg = -32765)'
        elseif hPopupEvent.Value == 4
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(1:rangeStartIdx,:) == -32765, 1, 'last');
        % if search for 'ECG Filter error (ecg = -32766)'
        elseif hPopupEvent.Value == 5
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(1:rangeStartIdx,:) == -32766, 1, 'last');
        % if search for 'ECG Lead off (ecg = -32767)'
        elseif hPopupEvent.Value == 6
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(1:rangeStartIdx,:) == -32767, 1, 'last');
        % if search for 'ECG Comm error (ecg = -32768)'
        elseif hPopupEvent.Value == 7
            [eventECG_rowIdx, eventECG_colIdx] = find(C3.ecg.data(1:rangeStartIdx,:) == -32768, 1, 'last');
        % if search for 'ECG abs(ecg) >= 5000 & < 10000'
        elseif hPopupEvent.Value == 8
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(1:rangeStartIdx,:)) >= 5000 & abs(C3.ecg.data(1:rangeStartIdx,:)) < 10000, 1, 'last');
        % if search for 'ECG abs(ecg) >= 10000 & < 20000'
        elseif hPopupEvent.Value == 9
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(1:rangeStartIdx,:)) >= 10000 & abs(C3.ecg.data(1:rangeStartIdx,:)) < 20000, 1, 'last');
        % if search for 'ECG abs(ecg) >= 20000'
        elseif hPopupEvent.Value == 10
            [eventECG_rowIdx, eventECG_colIdx] = find(abs(C3.ecg.data(1:rangeStartIdx,:)) >= 20000, 1, 'last');
        end
    end
end
